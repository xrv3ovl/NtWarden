...
bool    ImGui::BeginTabBar(const char* str_id, ImGuiTabBarFlags flags)
{
    ...
    float row_height = g.FontSize + g.Style.FramePadding.y * 2;
    int num_rows = (flags & ImGuiTabBarFlags_FittingPolicyWrap) ? ImMax((int)tab_bar->NumRows, 1) : 1;
    ImRect tab_bar_bb = ImRect(window->DC.CursorPos.x, window->DC.CursorPos.y, window->WorkRect.Max.x, window->DC.CursorPos.y + row_height * num_rows);
    tab_bar->ID = id;
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    // (Skip shrinking entirely when using wrap policy - tabs wrap to next row instead)
    const bool wrap_tabs = (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyWrap) != 0;
    ...
    if (width_excess >= 1.0f && !wrap_tabs && ((tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyResizeDown) || !central_section_is_visible))
    {
    ...
    int current_row = 0;
    float bar_width = tab_bar->BarRect.GetWidth();

    for (int section_n = 0; section_n < 3; section_n++)
    {
        ImGuiTabBarSection* section = &sections[section_n];
        if (section_n == 2 && !wrap_tabs)
            tab_offset = ImMin(ImMax(0.0f, bar_width - section->Width), tab_offset);

        for (int tab_n = 0; tab_n < section->TabCount; tab_n++)
        {
            ImGuiTabItem* tab = &tab_bar->Tabs[section_tab_index + tab_n];

            // Wrap to next row if this tab would exceed the bar width
            if (wrap_tabs && tab_offset + tab->Width > bar_width && tab_offset > 0.0f)
            {
                current_row++;
                tab_offset = 0.0f;
            }
    ...
    tab_bar->NumRows = (ImS16)(current_row + 1);
    ...
    // Update scrolling (disable when wrapping - all tabs are visible across rows)
    if (wrap_tabs)
    {
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    size.x = tab->Width;
    float row_y_offset = tab->Row * size.y;
    if (is_central_section)
        window->DC.CursorPos = tab_bar->BarRect.Min + ImVec2(IM_TRUNC(tab->Offset - tab_bar->ScrollingAnim), row_y_offset);
    else
        window->DC.CursorPos = tab_bar->BarRect.Min + ImVec2(tab->Offset, row_y_offset);
    ImVec2 pos = window->DC.CursorPos;
    ImRect bb(pos, pos + size);

    // We don't have CPU clipping primitives to clip the CloseButton (until it becomes a texture), so need to add an extra draw call (temporary in the case of vertical animation)
    // Skip clipping when wrapping - all tabs are visible (no scrolling)
    const bool is_wrapping = (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyWrap) != 0;
    const bool want_clip_rect = !is_wrapping && is_central_section && (bb.Min.x < tab_bar->ScrollingRectMinX || bb.Max.x > tab_bar->ScrollingRectMaxX);
    if (want_clip_rect)
