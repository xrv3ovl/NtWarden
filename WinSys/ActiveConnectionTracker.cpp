...
namespace {
	inline thread_local BYTE g_moduleInfoBuffer[1 << 10];

	void InitTcp4Connection(Connection* conn, const MIB_TCPROW_OWNER_MODULE& item) {
		DWORD size = sizeof(g_moduleInfoBuffer);
		auto moduleInfo = reinterpret_cast<PTCPIP_OWNER_MODULE_BASIC_INFO>(g_moduleInfoBuffer);
		...
		conn->State = static_cast<MIB_TCP_STATE>(item.dwState);
	}

	void InitTcp6Connection(Connection* conn, const MIB_TCP6ROW_OWNER_MODULE& item) {
		DWORD size = sizeof(g_moduleInfoBuffer);
		auto moduleInfo = reinterpret_cast<PTCPIP_OWNER_MODULE_BASIC_INFO>(g_moduleInfoBuffer);
	...
	void InitUdp4Connection(Connection* conn, const MIB_UDPROW_OWNER_MODULE& item) {
		DWORD size = sizeof(g_moduleInfoBuffer);
		auto moduleInfo = reinterpret_cast<PTCPIP_OWNER_MODULE_BASIC_INFO>(g_moduleInfoBuffer);
		...
		conn->State = static_cast<MIB_TCP_STATE>(0);
		conn->TimeStamp = item.liCreateTimestamp.QuadPart;
	}

	void InitUdp6Connection(Connection* conn, const MIB_UDP6ROW_OWNER_MODULE& item) {
		DWORD size = sizeof(g_moduleInfoBuffer);
		auto moduleInfo = reinterpret_cast<PTCPIP_OWNER_MODULE_BASIC_INFO>(g_moduleInfoBuffer);
	...
	void AddTcp4Connections(PMIB_TCPTABLE_OWNER_MODULE table, ActiveConnectionTracker::ConnectionMap& allConnections,
		ActiveConnectionTracker::ConnectionMap& map, ActiveConnectionTracker::ConnectionVec& current,
		ActiveConnectionTracker::ConnectionVec& newConnections, ActiveConnectionTracker::ConnectionVec& local, bool first) {
		...
		for (DWORD i = 0; i < count; i++) {
			...
			if (first) {
				...
				current.push_back(conn);
				allConnections.insert({ *conn, conn });
			}
			else {
				Connection key;
	...
	void AddTcp6Connections(PMIB_TCP6TABLE_OWNER_MODULE table, ActiveConnectionTracker::ConnectionMap& allConnections,
		ActiveConnectionTracker::ConnectionMap& map, ActiveConnectionTracker::ConnectionVec& current,
		ActiveConnectionTracker::ConnectionVec& newConnections, ActiveConnectionTracker::ConnectionVec& local, bool first) {
		auto count = table->dwNumEntries;
	...
	void AddUdp4Connections(PMIB_UDPTABLE_OWNER_MODULE table, ActiveConnectionTracker::ConnectionMap& allConnections,
		ActiveConnectionTracker::ConnectionMap& map, ActiveConnectionTracker::ConnectionVec& current,
		ActiveConnectionTracker::ConnectionVec& newConnections, ActiveConnectionTracker::ConnectionVec& local, bool first) {
		auto count = table->dwNumEntries;
		const auto& items = table->table;
		...
		for (DWORD i = 0; i < count; i++) {
			...
			if (first) {
				...
				if (auto it = map.find(key); it != map.end()) {
					...
					conn = std::make_shared<Connection>();
					InitUdp4Connection(conn.get(), item);
					allConnections.insert({ key, conn });
					newConnections.push_back(conn);
				}
				local.push_back(conn);
			}
	...
	void AddUdp6Connections(PMIB_UDP6TABLE_OWNER_MODULE table, ActiveConnectionTracker::ConnectionMap& allConnections,
		ActiveConnectionTracker::ConnectionMap& map, ActiveConnectionTracker::ConnectionVec& current,
		ActiveConnectionTracker::ConnectionVec& newConnections, ActiveConnectionTracker::ConnectionVec& local, bool first) {
		auto count = table->dwNumEntries;
		const auto& items = table->table;
...
int WinSys::ActiveConnectionTracker::EnumConnections() {
	...
	if ((_trackedConnections & ConnectionType::Tcp) == ConnectionType::Tcp) {
		...
		if (error == NOERROR) {
			...
			AddTcp4Connections(table, _connectionMap, map, _connections, _newConnections, local, first);
			::VirtualFree(buffer,0,MEM_RELEASE);
		}
	}
	if ((_trackedConnections & ConnectionType::TcpV6) == ConnectionType::TcpV6) {
		...
		if (error == NOERROR) {
			...
			AddTcp6Connections(table, _connectionMap, map, _connections, _newConnections, local, first);
			::VirtualFree(buffer, 0, MEM_RELEASE);
		}
	}
	if ((_trackedConnections & ConnectionType::Udp) == ConnectionType::Udp) {
		...
		if (error == NOERROR) {
			...
			AddUdp4Connections(table, _connectionMap, map, _connections, _newConnections, local, first);
			::VirtualFree(buffer, 0, MEM_RELEASE);
		}
	}
	if ((_trackedConnections & ConnectionType::UdpV6) == ConnectionType::UdpV6) {
		...
		if (error == NOERROR) {
			...
			AddUdp6Connections(table, _connectionMap, map, _connections, _newConnections, local, first);
			::VirtualFree(buffer, 0, MEM_RELEASE);
		}
	}
...
void ActiveConnectionTracker::Reset() {
	_connections.clear();
	_newConnections.clear();
	_closedConnections.clear();
