...
std::vector<std::shared_ptr<ServiceInfo>> ServiceManager::EnumServices(ServiceEnumType enumType) {
	std::vector<std::shared_ptr<ServiceInfo>> services;
...
//	auto QueryTagInformation = (PQUERY_TAG_INFORMATION)::GetProcAddress(::GetModuleHandle(L"advapi32"), "I_QueryTagInformation");
	...
	for (size_t i = 0; i < count; i++) {
		...
		std::shared_ptr<ServiceInfo> svc;
		svc = std::make_shared<ServiceInfo>();
